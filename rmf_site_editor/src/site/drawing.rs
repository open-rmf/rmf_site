...
use crate::{
    ...
    site::{
        get_current_workspace_path, Anchor, DefaultFile, FiducialMarker,
        GlobalDrawingVisibility, LayerVisibility, MeasurementMarker,
        MeasurementSegment, RecencyRank,
...
pub fn handle_loaded_drawing(
    ...
    for (entity, source, pose, pixels_per_meter, handle, vis, parent, rank) in loading_drawings.iter() {
        match asset_server.get_load_state(&handle.0) {
            LoadState::Loaded => {
                ...
                let default = parent.map(|p| default_drawing_vis.get(p.get()).ok()).flatten();
...
pub fn update_drawing_visibility(
    changed_drawings: Query<Entity, Or<(Changed<LayerVisibility>, Changed<Parent>, Changed<RecencyRank<DrawingMarker>>)>>,
    removed_vis: RemovedComponents<LayerVisibility>,
    all_drawings: Query<(Option<&LayerVisibility>, Option<&Parent>, Option<&RecencyRank<DrawingMarker>>, &DrawingSegments)>,
    material_handles: Query<&Handle<StandardMaterial>>,
    ...
    iter_update_drawing_visibility(
        changed_drawings.iter().filter_map(|e| all_drawings.get(e).ok()),
        &material_handles,
        &mut material_assets,
        &default_drawing_vis
    );
