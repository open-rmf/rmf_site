            ...
            .show(ui, |ui| {
                ui.horizontal_wrapped(|ui| {
                    ...
                    if ui.button("Copy Log History").clicked() {
                        ui.output_mut(|o| o.copied_text = self.events.display.log_history.copy_log_history());
                    }
...
fn print_log(ui: &mut egui::Ui, element: &LogHistoryElement) {
    ui.horizontal_wrapped(|ui| {
        ...
        if ui.selectable_label(false, msg).clicked() {
            ui.output_mut(|o| o.copied_text = element.log.category.to_string() + &element.log.message);
        }
