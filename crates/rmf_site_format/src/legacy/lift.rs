...
impl Lift {
    ...
    pub fn to_site(
        ...
        site_anchors: &mut BTreeMap<Entity, Anchor>,
        level_name_to_id: &BTreeMap<String, Entity>,
        all_lift_cabin_anchors: &BTreeMap<String, Vec<(Entity, Anchor)>>,
    ) -> Result<SiteLift> {
        let ref_anchor_positions = self.calculate_anchors();
        let reference_anchors = {
            let left = Entity::from_raw(site_id.next().unwrap());
            let right = Entity::from_raw(site_id.next().unwrap());
        ...
        let door_level_visits = {
            let mut door_level_visits: BTreeMap<Entity, LevelVisits> = BTreeMap::new();
        ...
        let mut cabin_anchors: BTreeMap<Entity, Anchor> = [all_lift_cabin_anchors.get(lift_name)]
            .into_iter()
            .filter_map(|x| x)
            .flat_map(|x| x)
            .cloned()
        ...
        let cabin_doors = {
            ...
            for face in RectFace::iter_all() {
                ...
                {
                    let left_id = Entity::from_raw(site_id.next().unwrap());
                    let right_id = Entity::from_raw(site_id.next().unwrap());
