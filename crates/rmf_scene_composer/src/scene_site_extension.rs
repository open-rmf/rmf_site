...
use crate::{SceneSubscriber, SceneSubscription};

use librmf_site_editor::site::{
    AssignSiteID, ExtensionHooks, ExtensionSettings, LevelElevation, LoadingArgs, Pose, SavingArgs,
    SetSiteExtensionHook, SiteID,
};
...
impl Plugin for SceneSiteExtensionPlugin {
    fn build(&self, app: &mut App) {
        app.world_mut().set_site_extension_hook(
            "gz-scene",
...
fn save_scenes(
    ...
    for level in children
        .get(site)
        .map_err(|_| SceneSavingError::InvalidSite)?
    {
...
fn load_scenes(
    ...
    for (id, info) in data {
        ...
        commands.entity(e).insert(info.pose).insert(ChildOf(*level));
    }
