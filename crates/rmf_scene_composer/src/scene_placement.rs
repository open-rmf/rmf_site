...
fn place_scene_2d_setup(
    ...
    commands
        .get_entity(state.scene_root)
        .or_broken_query()?
        .insert(ChildOf(cursor.frame));
...
fn place_scene_2d_cleanup(
    ...
    if let Ok(access) = access.get(&key) {
        if let Some(state) = access.newest() {
            ...
            commands.entity(state.scene_root).despawn();
        }
    }
...
fn on_scene_placement_chosen_2d(
    ...
    let Some(level) = current_level.0 else {
        return Err(Some(anyhow!(
            "No current level is active, unable to place scene"
        )));
    };
    ...
    scene.insert(ChildOf(level)).insert((pose, placement));
