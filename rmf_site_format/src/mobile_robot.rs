...
impl<T: RefTrait> Task<T> {
    pub fn convert<U: RefTrait>(&self, id_map: &HashMap<T, U>) -> Result<Task<U>, T> {
        Ok(match self {
            Task::GoToPlace(pose, parent) => Task::GoToPlace(pose.clone(), parent.convert(id_map)?),
            Task::WaitFor(time) => Task::WaitFor(*time),
            Task::PickUp(affiliation) => Task::PickUp(affiliation.convert(id_map)?),
            Task::DropOff(affiliation) => Task::DropOff(affiliation.convert(id_map)?),
        })
    }
}
...
// pub struct MobileRobotBundle<T, U: RefTrait> {
//     pub tasks: Tasks<U>,
//     pub drive: T,
// }
